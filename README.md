...
|---|---|---|---|---|
| **Lite (default)** | `fastembed` + Qdrant in-memory + SQLite Feast + FastAPI | `make setup-lite` (~60 s) | ~700 MB | Hầu hết học viên — laptop yếu, không Docker, focus vào concept |
| **Docker (full)** | Qdrant server + Redis + Postgres + bge-m3 | `make setup-docker` (~3 min) | ~3 GB | Học viên muốn trải nghiệm production stack thật, có Docker Desktop |
...
make setup-lite       # ~60 s — venv + deps + seed corpus + smoke test
...
Trên Windows, `make setup-lite` dùng PowerShell + `py -3`/`python` launcher để tạo venv.

Khi `make setup-lite` báo `All checks passed`, mở
...
```bash
make setup-docker       # ~3 min on first run (image pulls ~500 MB)
make api &
make benchmark
```
...
`make setup-lite` and `make lab` auto-convert to `.ipynb`. Edit `.ipynb` in
...
├── setup-lite.ps1                  # Windows-native lite bootstrap
├── setup-docker.sh                 # docker path bootstrap (full stack)
├── setup-docker.ps1                # Windows-native docker bootstrap
...
| Windows: `py` / `python` not found | Install Python 3.10+ and enable the Python launcher, then rerun `make setup-lite` |
